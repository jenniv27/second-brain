...
const ADD_FIELDS = [
  { key: 'definition', label: 'English',  placeholder: 'Hello',   required: true },
  { key: 'pinyin',     label: 'Pinyin',   placeholder: 'nǐ hǎo' },
]

const EDIT_FIELDS = [
  { key: 'hanzi',      label: 'Hanzi',    placeholder: '你好' },
  { key: 'pinyin',     label: 'Pinyin',   placeholder: 'nǐ hǎo' },
  { key: 'definition', label: 'English',  placeholder: 'Hello',   required: true },
]
...
export default function CardSearch({ cards, onAdd, onUpdate, onDelete }) {
  ...
  function openEdit(card) {
    ...
    setEditForm({ hanzi: card.hanzi ?? '', pinyin: card.pinyin ?? '', definition: card.definition ?? '' })
    setConfirmDel(false)
    setShowForm(false)
  }
  ...
  function handleAdd() {
    ...
    if (isDuplicate) { setFeedback('duplicate'); return }
  ...
  const canAdd  = form.definition.trim().length > 0
  const canSave = editForm.definition?.trim().length > 0
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <input
          value={query}
          onChange={e => { setQuery(e.target.value); setShowForm(false); setFeedback(null); closeEdit() }}
          placeholder="Search cards…"
          style={{
            flex: 1, border: 'none', outline: 'none', background: 'none',
            fontSize: '0.85rem', color: 'var(--text-dark)', fontFamily: 'inherit',
          }}
        />
        {query && (
                ...
                <div key={c.id}>
                  {editingId === c.id ? (
                    ...
                    <div style={{
                      ...
                      <p style={{ margin: '0 0 0.65rem', fontSize: '0.68rem', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--steel)' }}>
                        Edit card
                      </p>

                      {EDIT_FIELDS.map(({ key, label, placeholder }) => (
                        <div key={key} style={{ marginBottom: '0.55rem' }}>
                          <label style={{ display: 'block', fontSize: '0.68rem', color: 'var(--steel)', marginBottom: '0.18rem', letterSpacing: '0.04em' }}>
                            {label}
                          </label>
                          <input
                            value={editForm[key] ?? ''}
                            ...
                            style={{
                              ...
                              fontFamily: key === 'hanzi' ? 'serif' : 'inherit',
                            }}
                          />
                        </div>
                      ))}
                      ...
                      <div style={{ display: 'flex', gap: '0.45rem', marginTop: '0.65rem' }}>
                        <button
                          ...
                          style={{
                            ...
                            color: 'white', cursor: canSave ? 'pointer' : 'default',
                        ...
                        <button
                          ...
                          style={{
                            ...
                            border: '1px solid rgba(140,155,171,0.2)',
                            borderRadius: '0.65rem',
                            fontSize: '0.8rem', color: 'var(--steel)', cursor: 'pointer',
                          }}
                        >
                          Cancel
                        </button>
                        {confirmDel ? (
                          <>
                            <button
                              ...
                              style={{
                                ...
                                border: '1px solid rgba(192,99,90,0.3)',
                                borderRadius: '0.65rem',
                                fontSize: '0.8rem', fontWeight: 600,
                            ...
                            <button
                              ...
                              style={{ background: 'none', border: 'none', fontSize: '0.75rem', color: 'var(--steel)', cursor: 'pointer', padding: '0 0.25rem' }}
                            >
                              ✕
                            </button>
                          </>
                          ...
                          <button
                            ...
                            style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '0.5rem 0.4rem', display: 'flex', alignItems: 'center' }}
                            title="Delete card"
                          >
                            <Trash2 size={14} strokeWidth={1.75} color="var(--steel)" style={{ opacity: 0.5 }} />
                          </button>
                        )}
                      </div>
                    </div>
                    ...
                    <div style={{
                      ...
                      <button
                        onClick={() => openEdit(c)}
                        style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '0.2rem', display: 'flex', alignItems: 'center', flexShrink: 0 }}
                        title="Edit card"
                      >
                        <Pencil size={13} strokeWidth={1.75} color="var(--steel)" style={{ opacity: 0.45 }} />
                      </button>
                    </div>
                  )}
                </div>
